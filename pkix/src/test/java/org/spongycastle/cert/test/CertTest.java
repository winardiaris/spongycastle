    ...
    extends SimpleTest
{
        ...
        byte[] bytes)
    {
        ...
        try
        {
            ...
            isTrue(certHldr.isSignatureValid(new JcaContentVerifierProviderBuilder().setProvider("SC").build(k)));
            // System.out.println(cert);
        }
        catch (Exception e)
        {
        ...
        byte[] keyBytes)
    {
        ...
        try
        {
            ...
            KeyFactory keyFactory = KeyFactory.getInstance(k.getAlgorithm(), "SC");

            PrivateKey privKey = keyFactory.generatePrivate(new PKCS8EncodedKeySpec(keyBytes));

            Signature signer = Signature.getInstance(sigAlgorithm, "SC");
