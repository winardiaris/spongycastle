    ...
    extends SimpleTest
{
        ...
        throws Exception
    {
        ...
        try
        {
            ...
            PKCS10CertificationRequest csr = pkcs10Builder.build(
                new JcaContentSignerBuilder("SHA256WITHECDSA").setProvider("SC").build(enrollmentPair.getPrivate()));

            EnrollmentResponse enr = est.simpleEnroll(false, csr,
                new JcaHttpAuthBuilder("estreal", "estuser", "estpwd".toCharArray())
                    .setNonceGenerator(new SecureRandom()).setProvider("SC").build());
            ...
            enrolled.verify(expectedCA.getPublicKey(), "SC");
        ...
        throws Exception
    {
        ...
        try
        {
            ...
            EnrollmentResponse enr = est.simpleEnroll(false, csr, new HttpAuth("estreal", "estuser", "estpwd".toCharArray(), new SecureRandom(), new JcaDigestCalculatorProviderBuilder().setProvider("SC").build()));
        ...
        throws Exception
    {
        ...
        try
        {
            ...
            EnrollmentResponse enr = est.simpleEnroll(false, csr,
                new JcaHttpAuthBuilder("estuser", "estpwd".toCharArray()).setNonceGenerator(nonceRandom).setProvider("SC").build());
            X509Certificate expectedCA = ESTTestUtils.toJavaX509Certificate(ESTTestUtils.readPemCertificate(
                ESTServerUtils.makeRelativeToServerHome("/estCA/cacert.crt")
            ));
        ...
        try
        {
            ...
            try
            {
                EnrollmentResponse enr = est.simpleEnroll(
                    false,
                    csr,
                    new HttpAuth("estreal", "estuser", "estpwd".toCharArray(), new SecureRandom(), new JcaDigestCalculatorProviderBuilder().setProvider("SC").build()));
            }
            catch (Exception ex)
            {
...
//        ESTTestUtils.ensureProvider(BouncyCastleJsseProvider.PROVIDER_NAME);
        ...
        try
        {
            ...
            PKCS10CertificationRequestBuilder pkcs10Builder = new JcaPKCS10CertificationRequestBuilder(
                new X500Name("CN=Test"),
                enrollmentPair.getPublic());
            ContentSigner contentSigner = new JcaContentSignerBuilder("SHA256WITHECDSA").setProvider("SC").build(enrollmentPair.getPrivate());
            ...
            EnrollmentResponse enr = est.simpleEnrollPoP(
                ...
                pkcs10Builder,
                contentSigner,
                new JcaHttpAuthBuilder("estreal", "estuser", "estpwd".toCharArray()).setProvider("SC").setNonceGenerator(new SecureRandom()).build());
...
//        for (Iterator it = Security.getProvider("SC").getServices().iterator(); it.hasNext();)
//        {
